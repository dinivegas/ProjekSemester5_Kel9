...
class Chatkaprodi extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data = array(
            'data' => $this->db->query("SELECT * FROM `tb_chat` tc LEFT JOIN tb_kaprodi tk ON tc.id_kaprodi=tk.id_kaprodi 
            ...
            LEFT JOIN tb_psikolog tpsi ON tc.id_psikolog=tpsi.id_psikolog where  tc.id_kaprodi=".$this->session->userdata('userdata')['id_kaprodi'])->row_array()
        );
		// print_r($data);
        ...
        $this->load->view('user/chat_kaprodi', $data);
    }
    public function lanjutkan ($id){
        $this->db->update("tb_chat", ['status_chat' => 3], ['id_chat' => $id]);
        redirect(base_url('user/chat_kaprodi'));
    }
